      ...
      <div className="flex-1 relative bg-zinc-900 flex items-center justify-center">
        {callStatus === "calling" && !remoteStream && (
          <div className="text-center space-y-4 animate-pulse">
            ...
            >
              {remoteUser?.email?.slice(0, 2).toUpperCase() || 'NA'}
            </div>
            <h2 className="text-2xl font-bold">Calling {remoteUser?.email || 'User'}...</h2>
        ...
        {isVideoOff && (
          <div className="absolute inset-0 flex items-center justify-center bg-zinc-800">
            <div className="w-10 h-10 rounded-full bg-zinc-700 flex items-center justify-center">
              <span className="font-bold">{currentUser?.email?.slice(0, 2).toUpperCase() || 'NA'}</span>
            </div>
          </div>
        )}
      </div>
